...
## Domain-Specific Analysis (Research Harness)

During Phase 3 cross-run synthesis, the `research_harness/` module automatically segments every metric listed above by dataset domain (e.g. Academia, Legal) to detect topical performance drops. 
- **Domain Heatmaps**: Automatically generated by `visualizer.py` mapped to sequential confidence palettes.
- **Synthesized Traces**: Automatically shipped to the LLM backend by `synthesizer.py` (via `per_domain` Pydantic structs) enabling grounded paper-ready hypotheses on *why* models fail in specific verticals.
